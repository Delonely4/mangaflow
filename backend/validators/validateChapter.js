export const validateChapterData = ({
  book_id,
  number,
  title,
  release_date,
  ...
  if (typeof number !== "number" || number <= 0) {
    return {
      isValid: false,
      message: "Chapter number must be a positive number",
    };
  }
...
export const validateMultipleChapters = (chapters) => {
  ...
  if (chapters.length > 50) {
    return {
      isValid: false,
      message: "Cannot create more than 50 chapters at once",
    };
  }
  ...
  for (let i = 0; i < chapters.length; i++) {
    ...
    if (!chapter.number) {
      return {
        isValid: false,
        message: `Chapter ${i + 1}: number is required`,
      };
    }
    ...
    if (typeof chapter.number !== "number" || chapter.number <= 0) {
      return {
        isValid: false,
        message: `Chapter ${i + 1}: number must be a positive number`,
      };
    }
    ...
    if (chapter.title && typeof chapter.title !== "string") {
      return {
        isValid: false,
        message: `Chapter ${i + 1}: title must be a string`,
      };
    }
    ...
    if (chapter.release_date && isNaN(Date.parse(chapter.release_date))) {
      return {
        isValid: false,
        message: `Chapter ${i + 1}: invalid release date format`,
      };
    }
  }
