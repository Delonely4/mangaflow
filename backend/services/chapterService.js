...
export const getChaptersByBookId = async (bookId, userId = null) => {
  ...
  const readMap = new Map(read.map((r) => [String(r.chapter_number), r]));
...
export const updateChapter = async (chapterId, updateData) => {
  ...
  if (
    ...
    updateData.chapter_number !== existingChapter.chapter_number
  ) {
    ...
    if (duplicateChapter) {
      throw new Error(
        `Chapter ${updateData.chapter_number} already exists for this book`
      );
    }
  }
...
export const createMultipleChapters = async (book_id, chapters) => {
  ...
  if (existingChapters.length > 0) {
    ...
    throw new Error(
      `Chapters with numbers ${existingNumbers.join(", ")} already exist`
    );
  }
